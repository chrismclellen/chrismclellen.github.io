...
if ("undefined" === typeof window) {
  ...
  self.getReqStreamCmdStrTest = function () {
    ...
    var str = self.myprintf(
      "GET http://{0}:{1}/livestream/{2}?action=play&media={3} HTTP/1.1\r\n",
      hostip,
      hostport,
      streamtype,
      mediastr
  ...
  self.getWebServerIp = function (phost, pport) {
    ...
    if (pos1 >= 0) {
      ...
      if (pos1 >= 0) serveraddr.replace("http://", "ws://");
      else serveraddr = "ws://" + serveraddr;
      serveraddr += strport;
      //serveraddr += ':80';
      //serveraddr += ':8899';
    }
  ...
  self.getReqStreamCmdStr = function (
    phostip,
    pport,
    pstreamtype,
    puser,
    ...
    var str = myprintf(
      "GET http://{0}:{1}/livestream.cgi?stream={2}&action=play&media={3} HTTP/1.1\r\n",
      hostip,
      hostport,
      streamtype,
      mediastr
  ...
  HiNet.prototype.handler_header = function (obj, data) {
    ...
    if (ret !== -1) {
      ...
      self.postMessage({
        vtype: DATA_SYSTEMHEADER,
        picw: self.mWidth,
        pich: self.mHeight,
        codetype: DECCODE_265,
      });
    } else {
      // 264 ����
      console.log("recv video header");
  ...
  HiNet.prototype.connect = function (hostip, pport, puser, ppwd, pstreamtype) {
    ...
    this.ws.onopen = function (evt) {
      ...
      var str = self.getReqStreamCmdStr(
        hostip,
        pport,
        pstreamtype,
        puser,
    ...
    this.ws.onclose = function (evt) {
      ...
      that.ws.close();
      if (that.actcloseflag == 0) that.reconnectipc();
    };
    this.ws.onerror = function (evt) {
      console.log("websocket state:exception error");
    };
  ...
  HiNet.prototype.disconnect = function () {
    this.ws.close("1000", "");
    console.log("websock closeed");
    this.actcloseflag = 1;
    this.ws.close();
  };
        ...
        case "show":
          {
            ...
            if (self.g_showflag === 1) self.g_needIframe = true;
          }
          break;
      }
    },
  ...
  self._netcallback = function (data) {
    ...
    if (u32flag === 0x54565848) {
      ...
      console.log("recv video data");
      self.decworker.postMessage({ vtype: DATA_VIDEO_DATA, buf: data.buffer }, [
        data.buffer,
      ]);
      return;
    }
