...
if ("undefined" === typeof window) {
  ...
  self.Module = {
    onRuntimeInitialized: function () {
      ...
      self._Hi264DecCreate = that.module265.cwrap("Hi264DecCreate", "number", [
        "number",
        "number",
        "number",
      ]);
      self._Hi264DecDestroy = that.module265.cwrap(
        "Hi264DecDestroy",
        "undefined",
        ["number"]
      );
      self._Hi264DecFrame = that.module265.cwrap("Hi264DecFrame", "number", [
        "number",
        "number",
        "number",
        "number",
      ...
      self._Hi265DecCreate = that.module265.cwrap("Hi265DecCreate", "number", [
        "number",
        "number",
        "number",
      ]);
      self._Hi265DecDestroy = that.module265.cwrap(
        "Hi265DecDestroy",
        "undefined",
        ["number"]
      );
      self._Hi265DecFrame = that.module265.cwrap("Hi265DecFrame", "number", [
        "number",
        "number",
        "number",
        "number",
      ...
      self._memset = that.module265.cwrap("HiMemset", "undefined", [
        "number",
        "number",
        "number",
      ]);
  ...
  function cptr_copy_to_arraybuffer(cptr, dst_array, bytesLen) {
    ...
    var dv2 = new Uint8Array(
      that.module265.HEAPU8.subarray(cptr, cptr + bytesLen)
    );
  ...
  Decoder.prototype.onWasmLoaded = function () {
    ...
    this.decCallback = that.module265.addFunction(function (
      buff,
      size,
      nwidth,
      nheight,
  ...
  Decoder.prototype.Hi264DecFrame = function (
    hDecHandle,
    buf,
    len,
    pts,
    ...
    var ret = self._Hi264DecFrame(
      hDecHandle,
      self.g_dec_frame_buf,
      len,
      pts,
      self.g_dec_frame_s,
      uflags
  ...
  Decoder.prototype.Hi265DecFrame = function (
    hDecHandle,
    buf,
    len,
    pts,
  ...
  Decoder.prototype.getDecFrameInfo = function (cptr, decframeinfo) {
    var data = new Uint8Array(
      that.module265.HEAPU8.subarray(cptr, cptr + self.DECFRAME_LEN)
    );
  ...
  if (gtype === 0) {
    ...
    if (self.g_rec_pos === 0) {
      array_copy(
        self.g_rec_buf,
        0,
        self.g_buf_frame.payload,
        self.g_buf_frame.u32AVFrameLen
      );
    ...
    case DATA_SYSTEMHEADER:
      {
        ...
        if (self.g_rec_buf != null) self.g_rec_buf = null;

        self.g_rec_buf = new ArrayBuffer((ww * hh * 3) / 2);
        self.g_rec_pos = 0;

        console.log(
          "sysheader:codec-" + codetype + " widht=" + ww + " height=" + hh
        );
        console.log("create decoder...");
        if (self.decoder != null) {
          if (codetype === 1)
            self.g_decHandle = self.decoder.Hi265DecCreate(ww, hh);
          else self.g_decHandle = self.decoder.Hi264DecCreate(ww, hh);
  ...
  if (self.decoder) {
    if (!self.decoder.wasmLoaded) {
      ...
      if (self.g_codetype == 1) {
        self.g_decHandle = self.decoder.Hi265DecCreate(
          self.g_width,
          self.g_height
        );
      } else {
        self.g_decHandle = self.decoder.Hi264DecCreate(
          self.g_width,
          self.g_height
        );
      }
  ...
  if (self.q_video.get_count() >= 1 && self.g_decHandle != 0) {
    ...
    if (self.decoder != null) {
      if (self.g_codetype === 0) {
        var ret = self.decoder.Hi264DecFrame(
          self.g_decHandle,
          frame.payload,
          frame.u32AVFrameLen,
          frame.u32AVFramePTS,
        ...
        var ret = self.decoder.Hi265DecFrame(
          self.g_decHandle,
          frame.payload,
          frame.u32AVFrameLen,
          frame.u32AVFramePTS,
