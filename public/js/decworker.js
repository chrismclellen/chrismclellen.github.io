...
if ("undefined" === typeof window) {
  ...
  that.decoderFunc = that.g711_module.cwrap("G711Decoder", "number", [
    "number",
    "number",
    "number",
    "number",
  ]);
  that.encoderFunc = that.g711_module.cwrap("G711Encoder", "undefined", [
    "number",
    "number",
    "number",
    "number",
  ...
  function cptr_copy_from_arraybuffer_2(cptr, src_array, bytesLen) {
    ...
    var dst_dvt = new Uint8Array(
      g711_module.HEAPU8.subarray(cptr, cptr + bytesLen)
    );
  ...
  onmessage = function (event) {
    ...
    var frameP = new Uint8Array(
      audioG711data.subarray(headLen, headLen + AUDIO_FRAME_LEN)
    );
    ...
    var dv2 = new Uint8Array(
      that.g711_module.HEAPU8.subarray(
        short16Buf,
        short16Buf + 2 * AUDIO_FRAME_LEN
      )
    );
