...
HxPlayer.prototype.setShowFlag = function (vflag) {
  if (this.worker) this.worker.postMessage({ cmd: "show", showFlag: vflag });
};

HxPlayer.prototype.playvideo = function (
  hostip,
  nport,
  streamtype,
  struser,
  ...
  that.decworker.addEventListener(
    "message",
    function (e) {
      var data = e.data;
  ...
  this.hiAudioWorker.addEventListener(
    "message",
    function (e) {
  ...
  that.networker.addEventListener(
    "message",
    function (e) {
      var data = e.data;
      var infos;
        ...
        case DATA_AV_DATA:
          {
            ...
            if (frame.u32AVFrameFlag == 0x46565848) {
              ...
              that.decworker.postMessage(
                { vtype: DATA_VIDEO_DATA, buf: av_data },
                [av_data]
              );
            } else {
              //������Ƶ�̴߳���
  ...
  this.connect = function (url, nport, streamtype, struser, strpwd) {
    ...
    if ("MozWebSocket" in window) that.m_firefoxflag = 1;
    ...
    this.networker.postMessage({
      ...
      puser: struser,
      ppwd: strpwd,
      firefoxflag: that.m_firefoxflag,
